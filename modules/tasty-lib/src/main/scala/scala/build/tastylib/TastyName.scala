...
object TastyName {
  ...
  final case class QualifiedName(qual: TastyName, sep: SimpleName, selector: SimpleName)
      extends TastyName
  ...
  trait NameEncoder[U] {
    final def encode[O](name: TastyName)(init: => U, finish: U => O): O =
      finish(traverse(init, name))
  ...
  object SourceEncoder extends StringBuilderEncoder {
    def traverse(sb: StringBuilder, name: TastyName): StringBuilder = name match {
      ...
      case name: QualifiedName =>
        traverse(traverse(sb, name.qual) append (name.sep.raw), name.selector)
      ...
      case name: DefaultName =>
        traverse(sb, name.qual) append DefaultGetterStr append (name.num + 1)
    }
  }
  ...
  object DebugEncoder extends StringBuilderEncoder {
    ...
    def traverse(sb: StringBuilder, name: TastyName): StringBuilder = name match {
      ...
      case DefaultName(qual, num) =>
        traverse(sb, qual) append "[Default " append (num + 1) append ']'
      case PrefixName(prefix, qual) =>
        traverse(sb, qual) append "[Prefix " append (prefix.raw) append ']'
      case SuffixName(qual, suffix) =>
        traverse(sb, qual) append "[Suffix " append (suffix.raw) append ']'
      case ObjectName(name) => traverse(sb, name) append "[ModuleClass]"
      case TypeName(name)   => traverse(sb, name) append "[Type]"
      case SignedName(name, sig, target) => merge(
          traverse(sb, name) append "[Signed ",
          sig.map(_.signature)
        ) append " @" append target.source append ']'
      case QualifiedName(qual, sep, name) =>
        traverse(
          sb,
          qual
        ) append "[Qualified " append (sep.raw) append ' ' append (name.raw) append ']'
  ...
  def deepEncode(name: TastyName): TastyName = name match {
    ...
    case QualifiedName(qual, sep, selector) =>
      QualifiedName(deepEncode(qual), sep, deepEncode(selector).asSimpleName)
