...
object ResourceMapper {
  ...
  private def resourceMapping(build: Build.Successful): Map[os.Path, os.RelPath] =
    build.sources.resourceDirs
      .filter(os.exists(_))
      .flatMap { resourcePath =>
        if os.isFile(resourcePath) then
          Seq(resourcePath -> (os.rel / resourcePath.last))
        else if os.isDir(resourcePath) then
          os.walk(resourcePath)
            .filter(os.isFile(_))
            .map(path => path -> path.relativeTo(resourcePath))
        else Nil
      }
      // dismiss files generated by scala-cli
      .filterNot(_._2.startsWith(os.rel / Constants.workspaceDirName))
      .toMap
