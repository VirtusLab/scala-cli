...
object ResourceMapper {
  ...
  private def resourceMapping(build: Build.Successful): Map[os.Path, os.RelPath] = {
    val seq = for {
      ...
      resourceExactFilePath <- build.sources.resourceFiles.filter(os.isFile(_))
      relativeResourceExactFilePath = resourceExactFilePath.relativeTo(os.pwd)
      relativeResourcePath = resourceFilePath.relativeTo(resourceDirPath)
      _ = println("******************************************")
      _ = println(relativeResourceExactFilePath)
      _ = println("******************************************")
      // dismiss files generated by scala-cli
      if !relativeResourcePath.startsWith(os.rel / Constants.workspaceDirName)
    } yield Seq((resourceFilePath, relativeResourcePath), (resourceExactFilePath, relativeResourcePath))
