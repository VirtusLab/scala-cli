...
object ComputeVersion {
  ...
  final case class GitTag(
    ...
    def get(workspace: os.Path): Either[BuildException, String] = {
      def findRepo(dir: os.Path): Option[os.Path] =
        if dir.segmentCount <= 2 then None
        else if os.exists(dir / ".git") then Some(dir)
        else findRepo(dir / os.up)

      val baseRepo = repo.resolveFrom(workspace)

      findRepo(baseRepo) match {
        case Some(repo0) =>  
          Using.resource(Git.open(repo0.toIO)) { git =>
      ...
      case _ =>
        Left(new GitTagError(s"$baseRepo doesn't not seems to be inside any git repository."))
      }
    }
  }
  object GitTag {
