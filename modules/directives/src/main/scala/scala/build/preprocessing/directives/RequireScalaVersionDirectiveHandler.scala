...
object RequireScalaVersionDirectiveHandlers {
  ...
  abstract class Handler(suffixes: String*)(parse: String => VersionRequirement)
      extends BuildRequirementsHandler(Single(DirectiveValue.String, DirectiveValue.Number)) {
  ...
  val all = Seq(Matching, LowerThan, LowerThanOrEqual, HigherThan, HigherThanOrEqual)
}
