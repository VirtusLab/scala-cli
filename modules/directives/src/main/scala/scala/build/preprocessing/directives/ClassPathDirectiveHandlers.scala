...
object ClassPathDirectiveHandlers extends PrefixedDirectiveGroup[ClassPathOptions]("", "Class PATH", SharedDependencyOptions.help) {
  ...
  object Jar extends BaseStringListSetting {
    def exampleValues = Seq(
      Seq("/Users/alexandre/Library/Caches/Coursier/v1/https/repo1.maven.org/maven2/com/chuusai/shapeless_2.13/2.3.7/shapeless_2.13-2.3.7.jar"),
  ...
  object Libs extends BaseStringListSetting{
    def exampleValues = Seq(
      ...
      Seq("tabby:tabby:0.2.3,url=https://github.com/bjornregnell/tabby/releases/download/v0.2.3/tabby_3-0.2.3.jar")
    )
  ...
  object Repository extends BaseStringListSetting{
    ...
    def processOption(paths: ::[Positioned[String]])(using ctx: Ctx) = 
      paths.map(checkRepository).sequenceToComposite.map(r => ClassPathOptions(extraRepositories = r))
  } 
    ...
    def processOption(paths: ::[Positioned[String]])(using ctx: Ctx) = 
      osRootResource match 
        case Left(virtualRoot) =>
          val result = paths.map(p => p.safeMap(path => virtualRoot / os.SubPath(path), "Invalid path")).sequenceToComposite
          result.map(dirs => ClassPathOptions(resourcesVirtualDir = dirs.map(_.value)))
        case Right(root) =>
          val result = paths.map(_.safeMap(path => os.Path(path, root), "Invalid path")).sequenceToComposite
          for 
            paths <- result
            path <- paths if !os.exists(path.value)
          do ctx.logger.diagnostic("Provided resource directory path doesn't exist", positions = path.positions)
