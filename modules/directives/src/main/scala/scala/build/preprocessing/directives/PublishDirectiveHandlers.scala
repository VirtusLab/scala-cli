...
object PublishDirectiveHandlers extends PrefixedDirectiveGroup[PublishOptions](
      "publish",
      "Publish",
      scala.cli.commands.PublishOptions.help
    ) {
  def mkBuildOptions(v: PublishOptions): BuildOptions =
    BuildOptions(notForBloopOptions = PostBuildOptions(publishOptions = v))
  ...
  object ComputeVersion extends PublishSetting(
        ...
        override def valueFormat = "git:(dynver|tag)[:repo] | comamnd:<command>"
      }

  object URL extends PublishSetting(
        v =>
          v.safeMap(s => java.net.URI(s).toString, "Invalid URL").map(v =>
            PublishOptions(url = Some(v))
          ),
  ...
  object Developers extends BaseStringListSetting {
    def processOption(opts: ::[Positioned[String]])(using
      Ctx
    ): Either[BuildException, PublishOptions] =
      opts.map(publish.Developer.parse).sequenceToComposite.map(parsed =>
        PublishOptions(developers = parsed)
      )

    def exampleValues = Seq(
      Seq("_example|Sarah Jones|https://docs.github.com/_example"),
      Seq(
        "_example|Sarah Jones|https://docs.github.com/_example",
        "_example2|Nick Smith|https://docs.github.com/_example|nick@example.org"
      )
    )
