...
case object UsingPlatformDirectiveHandler
    extends BuildOptionsHandler(AtLeastOne(DirectiveValue.String)) {
  def name             = "Platform"
  ...
  def process(values: ::[Positioned[String]])(using ctx: Ctx) = {
    ...
    parsed.flatMap { values =>
      ...
      val problems = byPlatform.filter(_._2.size > 1).map { case (platform, values) =>
        ...
        def warn(p: Positioned[_]): Unit = ctx.logger.diagnostic(
          s"Duplicated definition for platform $platform",
          positions = p.positions
        )
        versioned match
          case Seq() =>
            Right(values.tail.foreach(warn)) // multiple directives without version, just warn
          case Seq(single) =>
            Right(
              noVersion.tail.foreach(warn)
            ) // single directice with version rest without - warn on those whitout version
          case declaration +: duplications =>
            duplications.map(
              _.error(s"Version for $platform is alredy defined")
            ).sequenceToComposite
      }.toSeq.sequenceToComposite

      problems.map { _ =>
        val mainPlatfrom = values.head.map(_.platform)
        val extraPlatforms = (byPlatform - mainPlatfrom.value).map { (k, v) =>
          k -> v.head.map(_ => ())
        }
        ...
        BuildOptions(
          scalaOptions =
            ScalaOptions(platform = Some(mainPlatfrom), extraPlatforms = extraPlatforms),
