...
Pass scalafmt version before running it (3.7.3 by default). If passed, this overrides whatever value is configured in the .scalafmt.conf file.
...
Set the Ammonite version (2.5.8 by default)
...
Aliases: `--class`, `--classes`, `--extra-class`, `--extra-class-path`, `--extra-classes`, `--extra-jar`, `--jar`, `--jars`

Add extra JARs and compiled classes to the class path

### `--class-path`

Aliases: `--classpath`, `-classpath`, `-cp`
