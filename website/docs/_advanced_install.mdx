...
```
You now have scala-cli installed in your system, but currently support is partial. Scala CLI is installed as a JAR rather than a native binary, and so it has a slightly higher startup time.

To run scala-cli anywhere on your system, you need to add its binary path to the $PATH variable. **Assuming your user directory is foobar**:
```bash
...
To build your projects successfully, remember to add ```--server=false``` to your CLI arguments, otherwise Scala CLI fails to work on FreeBSD.

Any functionality that fetches JVMs from third parties (such as ```--jvm``` and ```package --native-image```) will not work on FreeBSD, as these third parties don't provide FreeBSD builds.
</TabItem>
