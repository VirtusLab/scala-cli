...
FreeBSD support (and support for other BSDs) is currently very partial. Scala CLI does not distribute for FreeBSD, but you can install it via Coursier's standalone JAR.
...
```bash
#Get coursier
curl https://github.com/coursier/coursier/raw/gh-pages/coursier
#Add executable permissions and run Coursier to install scala-cli
#Coursier's setup is not available on FreeBSD
chmod +x coursier
./coursier install scala-cli

#Add scala-cli to your $PATH (add the command to your shell's RC file to make the setting permanent)
#Assuming your home user's directory is "foobar"
export PATH="$PATH:/home/foobar/.local/share/coursier/bin"
```
You now have scala-cli installed in your system, but currently support is partial.

To build your projects successfully, remember to add ```--server=false``` to your CLI arguments.

Any functionality that fetches JVMs from third parties (such as ```--jvm``` and ```package --native-image```) will not work on FreeBSD.

Scala CLI is installed as a JAR rather than a native binary, and so it has a slightly higher startup time.
