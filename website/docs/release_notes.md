...
To exclude specific source files or entire directories from a Scala CLI project using the exclude directive in your `project.scala` file or the command line parameter `--exclude`, along with a pattern:
...
- removed the possibility of nesting from the API.
...
The Scala CLI launcher is available as a standalone fat JAR. You can download the nightly version of the Scala CLI fat JAR from Sonatype and try it now:
...
With the special `scriptPath` function, you can now easily access the name of the running script inside the script itself. Example of how to use the scriptPath variable:
