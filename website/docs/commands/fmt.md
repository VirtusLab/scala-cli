...
It is possible to pass configuration as a string directly from the command line, using `--scalafmt-conf-str` option. If the configuration is passed this way, Scala CLI behaves exactly the same as it would find `.scalafmt.conf` with specified configuration in the workspace.
...
:::note
If the configuration is passed in the `--scalafmt-conf-str` option, Scala CLI behaves exactly the same as it would find `.scalafmt.conf` with specified configuration in the workspace.
:::