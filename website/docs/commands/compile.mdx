...
<RunnableCommand
    command={"scala-cli compile Hello.scala"}
    output={`Hello`}
/>
...
<RunnableCommand
    command={"scala-cli compile --watch Hello.scala"}
    output={
...
<RunnableCommand
    command={"scala-cli compile Hello.scala -Xlint:infer-any"}
    output={
...
<RunnableCommand
    command={"scala-cli compile Hello.scala -O -deprecation -O -Xlint:infer-any"}
    output={
...
<RunnableCommand
    command={"scala-cli compile --class-path Hello.scala"}
    output={
...
<RunnableCommand
    command={`java -cp "$(scala compile --class-path Hello.scala)" Hello`}
    output={"Hello"}
    outputLanguage="bash"
/>

Note that you should favor the [`run`](./run.mdx) command to run your code, rather than running `java -cp`.
