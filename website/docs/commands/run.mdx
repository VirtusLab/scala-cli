...
<RunnableCommand
    command={"scala-cli run Hello.scala"}
    output={"Hello"}
/>
...
<RunnableCommand
    command={"scala-cli app.sc -- first-arg second-arg"}
    output={"App called with arguments: first-arg, second-arg"}
    outputLanguage="text"
/>
...
<RunnableCommand
    command={"scala-cli run HelloScript.sc"}
    output={"Hello world from scala script"}
/>

Our [scripts guide](../guides/scripts.mdx) provides many more details.
...
<RunnableCommand
    command={"docker run  -v $(pwd)/HelloWorld.scala:/HelloWorld.scala virtuslab/scala-cli /HelloWorld.scala"}
    output={"Hello world"}
/>
